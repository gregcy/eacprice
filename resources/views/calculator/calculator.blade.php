<form id="eac-calculator" class="p-6 mb-12" method="POST" action="{{ route('calculator.calculate',['lang' => app()->getLocale()]) }}">
    ...
    <fieldset id="tariff" class="py-4 w-100">
        <label for="tariff-select" class="text-lg font-medium pr-4 w-full block sm:inline sm:w-auto">{{ __('Tariff') }}:</label>
        <select id="tariff-select" name="tariff"
            class="inline-block grow border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm w-full block sm:inline sm:w-auto">
            @php
                $options = [
        ...
        <label for="period" class="text-lg font-medium pr-4 w-full block sm:inline sm:w-auto">{{ __('Month') }}:</label>
    ...
    <fieldset id="tariff01" class="{{ (isset($values) && in_array($values['tariff'], ['01', '08'])) || !isset($values) ? 'block' : 'hidden' }}">
        <div class="w-100">
            <label for="consumption" class="text-lg font-medium pr-20 w-full block sm:inline sm:w-auto">{{ __('Consumption') }} (kWh):</label>
        ...
        <div class="w-100 py-4">
            <label for="credit-amount" class="text-lg font-medium pr-4 w-full block sm:inline sm:w-auto">{{ __('Returned Solar Power') }} (kWh):</label>
    ...
    <fieldset id="tariff02" class="{{ (isset($values) && $values['tariff'] === '02') ? 'block' : 'hidden' }}">
        <div class="w-100">
            <label for="consumption-standard" class="text-lg font-medium pr-5 w-full block sm:inline sm:w-auto">{{ __('Consumption During Standard Period') }} 09:00-23:00 (kWh):</label>
        ...
        <div class="w-100 py-4">
            <label for="consumption-economy" class="text-lg font-medium pr-4 w-full block sm:inline sm:w-auto">{{ __('Consumption During Economy Period') }} 23:00-09:00 (kWh):</label>
